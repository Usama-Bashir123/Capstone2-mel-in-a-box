...
// Edit Child Profile — Wired to Firestore + Firebase Storage
// Loads existing child data from users/{uid}/children/{id}
// Updates with updateDoc; replaces avatar in Storage if new file provided
...
export default function EditChildPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const fetchChild = async () => {
      try {
        const docSnap = await getDoc(doc(db, "users", user.uid, "children", params.id));
        if (docSnap.exists()) {
          ...
          setForm({
            ...
            contentType: data.contentType || "Both",
          });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Upload new avatar to Firebase Storage if a new file was selected
      if (avatarFile) {
        try {
          ...
          setError(
            `Photo upload failed (${code}). Profile info was saved without the new photo. ` +
            `Fix Storage rules in Firebase Console → Storage → Rules and try again.`
          );
          // Fall through — save profile without new photo
        }
      }

      // Always update Firestore (even if photo upload failed)
      await updateDoc(doc(db, "users", user.uid, "children", params.id), {
        name: form.name.trim(),
        dob: form.dob,
        gender: form.gender,
  ...
  const displayPhoto = avatarPreview || currentPhotoURL;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <Link href={`/parent/children/${params.id}`} className="font-nunito font-bold" style={{ fontSize: "14px", color: "#424242", textDecoration: "none" }}>
            {form.name || "Profile"}
          </Link>
          <ChevronRight size={16} style={{ color: "#737373" }} />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <button
            ...
            style={{ padding: "10px 14px", borderRadius: "8px", border: "1px solid #F63D68", background: "#F63D68", fontSize: "14px", boxShadow: "0px 1px 2px rgba(16,24,40,0.05)", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", opacity: isSubmitting ? 0.7 : 1 }}
          >
      ...
      <SectionCard title="Child Information">
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          <FormInput label="Full Name" placeholder="Enter full name" value={form.name} onChange={set("name")} required />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px", alignItems: "center" }}>
            <div style={{ width: "80px", height: "80px", borderRadius: "12px", overflow: "hidden", border: "1px solid #E5E5E5", background: "#FFF1F3", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {displayPhoto ? (
                ...
                <img src={displayPhoto} alt="Current avatar" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              ) : (
                <span style={{ fontSize: "36px" }}>🧒</span>
              )}
            </div>
            <span className="font-nunito font-normal" style={{ fontSize: "12px", color: "#737373", textAlign: "center" }}>
              {avatarPreview ? "New" : "Current"}
            </span>
          </div>

          {/* Drop zone */}
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "8px" }}>
            <p className="font-nunito font-semibold" style={{ fontSize: "14px", color: "#424242" }}>
              {avatarFile ? "New avatar selected" : "Change avatar"}
            </p>
            <p className="font-nunito font-normal" style={{ fontSize: "14px", color: "#737373" }}>Upload a new image to replace the current avatar.</p>
            <div
