...
export default function EditVideoPage() {
  ...
  const onSave = async (status: "Published" | "Draft") => {
    ...
    try {
      ...
      await logActivity({
        type: "Video",
        activity: `Updated video "${form.name}" — ${status === "Draft" ? "saved as draft" : "published"}`,
  ...
  if (isLoading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "400px", gap: "12px" }}>
        <Loader2 size={24} style={{ color: "#F63D68", animation: "spin 1s linear infinite" }} />
        <p className="font-nunito font-normal" style={{ fontSize: "14px", color: "#525252" }}>Loading video...</p>
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "400px", gap: "16px" }}>
        <p className="font-nunito font-semibold" style={{ fontSize: "20px", color: "#141414" }}>Video not found</p>
        <Link href="/admin/videos" style={{ color: "#F63D68", fontSize: "14px", textDecoration: "none", fontWeight: 700 }} className="font-nunito font-bold">
          Back to Videos
        </Link>
      </div>
    );
